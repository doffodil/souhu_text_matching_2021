...
import numpy as np
import os
from process import Dataset, stopwords_loader
...
# bert配置
config_path = 'NEZHA-Base-WWM/bert_config.json'
checkpoint_path = 'NEZHA-Base-WWM/model.ckpt-691689'
dict_path = 'NEZHA-Base-WWM/vocab.txt'
...
for i, var in enumerate(var_list):
    ...
    for file_name in train_file_name_list:
        train_data.merge_dataset(Dataset.dataset_loader(file_name, i))
    # 加载测试集
    file_name = 'datasets/sohu2021_open_data_clean/%s/test_with_id.txt' % var
    test_data.merge_dataset(Dataset.dataset_loader(file_name, i))
    # 加载验证集
    valid_file_name_list = [
        'datasets/sohu2021_open_data_clean/%s/valid.txt' % var,
        'datasets/rematch/%s/valid.txt' % var
    ]
    for file_name in valid_file_name_list:
        valid_data.merge_dataset(Dataset.dataset_loader(file_name, i))

# 数据预处理
train_data.clean_dataset()
valid_data.clean_dataset()
test_data.clean_dataset()

# 数据增强
train_data.pool_enhance_dataset()
...
c_in = Input(shape=(1,))
c = Embedding(len(var_list), 128)(c_in)
c = Reshape((128,))(c)
...
# 转换数据集
train_generator = data_generator(train_data.sample_list, batch_size)
valid_generator = data_generator(valid_data.sample_list, batch_size)
test_generator = data_generator(test_data.sample_list, batch_size)
